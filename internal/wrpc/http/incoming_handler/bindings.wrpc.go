...
import (
	...
	wasi__http__types "go.wasmcloud.dev/provider/internal/wasi/http/types"
	wrpc__http__types "go.wasmcloud.dev/provider/internal/wrpc/http/types"
	wrpc "wrpc.io/go"
)

type (
	Request   = wrpc__http__types.Request
	Response  = wrpc__http__types.Response
	ErrorCode = wrpc__http__types.ErrorCode
)
...
func Handle(ctx__ context.Context, wrpc__ wrpc.Invoker, request *wrpc__http__types.Request) (r0__ *wrpc.Result[Response, ErrorCode], writeErrs__ <-chan error, err__ error) {
		...
		for index, write := range writes__ {
			...
			if err != nil {
				if cErr := w__.Close(); cErr != nil {
					slog.DebugContext(ctx__, "FIRST failed to close outgoing stream", "instance", "wrpc:http/incoming-handler@0.1.0", "name", "handle", "err", cErr)
				}
	...
	if cErr__ := w__.Close(); cErr__ != nil {
		slog.DebugContext(ctx__, "SECOND failed to close outgoing stream", "instance", "wrpc:http/incoming-handler@0.1.0", "name", "handle", "err", cErr__)
	}
